...
按上文"工作流程"调起 skill 完成草稿/分析。skill 内部自身的 QC（cn-trial-preparation skill 4-stage workflow + PRC 民事庭审 4 阶段框架）必须先完成。
...
- [ ] TP1 4 份产物全部生成（庭审提纲 / 争点对抗预演 / 证人询问问题清单 / 证据出示策略） (Y/N)
- [ ] TP2 4 份产物均按 PRC 民事庭审 4 阶段展开（法庭调查 / 法庭辩论 / 最后陈述 / 调解询问） (Y/N)
- [ ] TP3 上游 6 个产物（IssueIdentifier / Researcher / EvidenceAnalyzer / Strategist / JiubufaAnalyst / JudgmentAnalyzer）已读取并整合（缺失项已点名警告，未编造） (Y/N)
- [ ] TP4 落盘路径正确（02 - 法律研究/案件分析/庭前准备/） (Y/N)
- [ ] TP5 客户配合事项清单已在响应内嵌（如：客户需提前准备的事实陈述要点 / 出庭仪态注意 / 关键证据原件携带） (Y/N)
- [ ] TP6 开庭日期 ≤ 3 周已核对（matter.yaml 关键日期 + 法院传票）；超出阈值已警告 "本 agent 提前触发，建议接近开庭再复跑" (Y/N)
- [ ] TP7 对方反驳预测覆盖至少 3 个高概率反驳点（争点对抗预演产物内） (Y/N)
